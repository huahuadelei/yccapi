...
public class GlobalExceptionHandler implements InitializingBean {
    ...
    public void handleException(Exception e, HttpServletRequest request, HttpServletResponse response){
        ...
        if(e instanceof YccException){
            ...
            ResultBean resultBean = ResultBean.builder().code("400").msg("未携带请求体数据").build();
            WebHttpUtils.writeCoutomResultBeanData(response,resultBean);

        }else if (e instanceof HttpRequestMethodNotSupportedException || e instanceof MissingServletRequestParameterException){
            ...
            ResultBean resultBean = ResultBean.builder().code("500").msg("系统发生异常").build();
            WebHttpUtils.writeCoutomResultBeanData(response,resultBean);
        }
    }
    ...
    private void handlerAuthorizationException(HttpServletResponse response, AuthorizationException e) {
        if(e instanceof UnauthorizedException){
            ResultBean resultBean = ResultBean.builder().code("403").msg("您没有权限操作").build();
            WebHttpUtils.writeCoutomResultBeanData(response,resultBean);
