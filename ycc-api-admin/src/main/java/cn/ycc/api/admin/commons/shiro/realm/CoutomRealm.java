...
public class CoutomRealm extends AuthorizingRealm {
    ...
    private LoginUser loadLoginUserInfo(String userId) {
        ...
        List<YccMenu> permList = menuMapper.selectMenuPermsByRoleId(yccRole.getId(),0);
        Set<String> permissionSet = permList.stream().map(YccMenu::getPermCode).collect(Collectors.toSet());

        List<YccMenu> yccMenus = menuMapper.selectMenuPermsByRoleId(yccRole.getId(),1);
